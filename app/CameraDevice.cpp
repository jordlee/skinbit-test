...
void CameraDevice::speed_test_gpio_hardware_trigger()
{
    using namespace std::chrono;

    tout << "\n";
    tout << std::string(80, '=') << "\n";
    tout << "ILX-LR1 Speed Test - GPIO Hardware Trigger (SD Card + Wait for Focus)\n";
    tout << std::string(80, '=') << "\n\n";
    ...
    std::string log_filename = std::string("speed_test_gpio_") + timestamp + ".log";
    ...
    log("\n");
    log(std::string(80, '=') + "\n");
    log("ILX-LR1 Speed Test - GPIO Hardware Trigger\n");
    ...
    log("Starting capture sequence (GPIO hardware trigger with focus wait)...\n\n");
    ...
    for (int i = 1; i <= TOTAL_PHOTOS; i++) {
        ...
        auto focus_wait_elapsed = duration_cast<milliseconds>(after_focus_ready - after_focus_set).count();
        ...
        photo_msg << "Photo " << i << "/" << TOTAL_PHOTOS
                  << ": Focus=0x" << std::hex << current_focus << std::dec
                  << ", Set@" << elapsed_ms << "ms"
                  << ", Wait@" << (elapsed_ms + focus_set_elapsed + focus_wait_elapsed) << "ms"
                  << ", GPIO_Press@" << (elapsed_ms + focus_set_elapsed + focus_wait_elapsed + press_elapsed) << "ms"
                  << ", Cycle=" << cycle_time << "ms\n";
        log(photo_msg.str());
